...
class DevelopmentCommandSet : CommandSet(Privilege.ADMIN) {
    override fun defineCommands() {
        /*
         * Command for remove gravedigger attributes.
         */
        ...
        define(
            ...
            usage = "::resetgd",
            description = "Reset attributes for gravedigger random event.",
        ) { player, _ ->
            GravediggerListener.reset(player)
            player.debug("Attributes cleaned.")
        }

        /*
         * Command for randomize gravedigger attributes.
         */
        ...
        define(
            ...
            GravediggerListener.reset(player)
            runTask(player, 1 ) {GravediggerListener.init(player)}
            player.debug("Done.")
        }

        /*
         * Command for check gravedigger attributes.
         */
        ...
        define(
            ...
            privilege = Privilege.ADMIN,
            usage = "::checkgraves",
            description = "Open interface and draw attributes for gravedigger random event.",
        ) { player, _ ->
            openInterface(player, Components.MESSAGESCROLL_220).also {
                sendString(player, "Gravedigger attributes table", Components.MESSAGESCROLL_220, 4)

                val header = String.format("%-15s | %-12s | %-15s | %-12s | %-15s", "SET", "COFFIN", "GRAVESTONE", "GRAVE", "EMPTY GRAVE")
                val divider = "_".repeat(header.length)

                var line = 5
                sendString(player, divider, Components.MESSAGESCROLL_220, line++)
                sendString(player, header, Components.MESSAGESCROLL_220, line++)
                sendString(player, divider, Components.MESSAGESCROLL_220, line++)

                GravediggerListener.COFFIN_SETS.forEach {
                    val name = it.name
                    val coffin = player.getAttribute("coffin_id:$name", -1)
                    val gravestone = player.getAttribute("gravestone_id:$name", -1)
                    val grave = player.getAttribute("grave_id:$name", -1)
                    val emptyGrave = player.getAttribute("empty_grave_id:$name", -1)

                    val row = String.format("%-15s | %-12d | %-15d | %-12d | %-15d", name, coffin, gravestone, grave, emptyGrave)
                    sendString(player, row, Components.MESSAGESCROLL_220, line++)
                }

                sendString(player, divider, Components.MESSAGESCROLL_220, line++)
                val score = player.getAttribute(GameAttributes.GRAVEDIGGER_SCORE, 0)
                sendString(player, "Current score: [$score] | Randomize command: [::randgd] | Reset command: [::resetgd]" , Components.MESSAGESCROLL_220, line)
            }
            return@define
        }
