...
public class Region {
	...
	public static void load(Region region, boolean build) {
		try {
			...
			String mapscapeName = "m" + regionX + "_" + regionY;
			int mapscapeId = Cache.getArchiveId(CacheIndex.LANDSCAPES, mapscapeName);
			...
			if (mapscapeId >= 0) {
				byte[] mapscapeBytes = Cache.getData(CacheIndex.LANDSCAPES, mapscapeName);
			...
			region.setHasFlags(dynamic);
			region.setLoaded(true);

			String landscapeName = "l" + regionX + "_" + regionY;
			int[] xtea = XteaParser.Companion.getRegionXTEA(regionId);
			byte[] landscapeBytes = Cache.getData(CacheIndex.LANDSCAPES, landscapeName, xtea);

			if (landscapeBytes == null || landscapeBytes.length < 4) {
				return;
			}

			region.setHasFlags(true);

			try {
				LandscapeParser.parse(region, mapscapeData, ByteBuffer.wrap(landscapeBytes), build);
			} catch (Throwable t) {
