    ...
    public void add(GroundItem item) {
        ...
        if (g.isPrivate()) {
            if (g.getDropper() != null) {
                PacketRepository.send(ConstructGroundItem.class, new BuildItemContext(g.getDropper(), item, 0));
            }
            return;
        }
    ...
    public void remove(GroundItem item) {
        ...
        if (item.isPrivate()) {
            if (item.getDropper() != null && item.getDropper().isPlaying() && item.getDropper().getLocation().withinDistance(l)) {
                PacketRepository.send(ClearGroundItem.class, new BuildItemContext(item.getDropper(), item, 0));
            }
            return;
        }
