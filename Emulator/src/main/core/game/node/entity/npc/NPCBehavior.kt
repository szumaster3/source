...
open class NPCBehavior(vararg val ids: Int = intArrayOf()) : ContentInterface {
     ...
     * @return whether we should proceed with the base NPC tick() method - e.g.
     * returning false means we do not proceed with a normal NPC tick.
     */
    ...
    /**
     * Called before this NPC receives damage, and allows you to adjust the
     * battlestate if needed.
     * @param self the NPC instance this behavior belongs to
    ...
    /**
     * Called after this NPC receives damage, and allows you to adjust the
     * battlestate if needed.
     * @param self the NPC instance this behavior belongs to
     * @param attacker the entity attacking this NPC
     * @param state the current state of the combat between this NPC and the attacker.
    ...
    /**
     * Called after this NPC's basic attack has been calculated, but before it is
     * finalized, so adjustments can be made.
     * @param self the NPC instance this behavior belongs to
     ...
     * Note: This is not the same as death. Death does not remove an NPC,
     * unless that NPC cannot respawn.
     * @param self the NPC instance this behavior belongs to
     */
    ...
    /**
     * Called immediately when the NPC first begins to die (on the same tick that the
    ...
    /**
     * Called immediately after the death animation of this NPC has finished, on the same
     * tick that drop tables are rolled.
     * @param self the NPC instance this behavior belongs to
     * @param killer the entity which killed this NPC.
     */
    ...
    /**
     * Called after this NPC's drop table is rolled, but before the items are actually dropped,
     * so the list can be manipulated.
     * @param self the NPC instance this behavior belongs to
     ...
     * @param shouldSendMessage whether the core combat code believes you should send a message e.g.
     * "You can't attack this NPC with that weapon"
    ...
    /**
     * Called by combat-related code to check if this NPC should ignore multi-combat rules when
     * attempting to attack the given victim.
     * @param self the NPC instance this behavior belongs to
    ...
    /**
     * Called by MovementPulse to determine if a non-default Pathfinder should be used (e.g.
     * whether this npc should intelligently walk around obstacles)
     */
    ...
    /**
     * Called by pathfinding code to determine the clipping mask supplier this NPC should use.
     * You can use this to ignore water, etc.
     */
