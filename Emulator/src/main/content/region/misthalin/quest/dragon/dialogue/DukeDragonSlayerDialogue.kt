...
class DukeDragonSlayerDialogue(
    val questStage: Int,
) : DialogueFile() {
    override fun handle(
        componentID: Int,
        ...
        if (questStage in 20..99 && !hasShield(player!!)) {
            when (stage) {
                ...
                1 ->
                    npc(
                        "A knight going on a dragon quest, hmm? What",
                        "dragon do you intend to slay?",
                    ).also { stage++ }
            ...
            when (stage) {
                ...
                1 ->
                    npc("If you kill it, for Saradomin's sake make sure it's really", "dead!").also {
                        stage = END_DIALOGUE
                    }
            }
            ...
            when (stage) {
                ...
                3 ->
                    npc(
                        "Of course. Now you've slain Elvarg, you've earned",
                        "the right to call the shield your own!",
                    ).also { stage++ }
