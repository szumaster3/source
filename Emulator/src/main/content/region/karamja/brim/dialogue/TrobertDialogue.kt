package content.region.karamja.brim.dialogue
...
private class TrobertDialogueFile : DialogueBuilderFile() {
    override fun create(b: DialogueBuilder) {
        b.onQuestStages(Quests.HEROES_QUEST, 0, 1)
            ...
            .playerl("Pleased to meet you.").npcl("Likewise.").end()

        b.onQuestStages(Quests.HEROES_QUEST, 2)
            .npcl("Welcome to our Brimhaven headquarters. I'm Trobert and I'm in charge here.").options()
            .let { optionBuilder ->
                val continuePath = b.placeholder()
                optionBuilder.option("So can you help me get Scarface Pete's candlesticks?").goto(continuePath)
                optionBuilder.option_playerl("Pleased to meet you.").npcl("Likewise.").goto(continuePath)
                return@let continuePath.builder()
            }.playerl("So can you help me get Scarface Pete's candlesticks?").npcl(
                ...
                optionBuilder.option_playerl("I volunteer to undertake that mission!").npcl(
                ...
                optionBuilder.option_playerl("Well, good luck then.").npcl("Someone will show up eventually.").end()
            }

        b.onQuestStages(Quests.HEROES_QUEST, 3, 4, 5).branch { player ->
            ...
            branch.onValue(1).npcl("How's it going?").playerl("Fine, thanks.").end()
            branch.onValue(0).playerl("I have lost Hartigen's ID papers.").npcl(
        ...
        b.onQuestStages(Quests.HEROES_QUEST, 6, 100).npcl("How's it going?").playerl("Fine, thanks.").end()
    }
}
