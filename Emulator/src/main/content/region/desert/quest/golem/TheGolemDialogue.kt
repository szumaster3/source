...
class ClayGolemDialogueFile : DialogueBuilderFile() {
    override fun create(b: DialogueBuilder) {
        val opt1 = b.onQuestStages(Quests.THE_GOLEM, 0)
            .npc(FaceAnim.OLD_BOWS_HEAD_SAD, "Damage... severe...", "task... incomplete...").options()
        opt1.optionIf("Shall I try to repair you?") { player ->
            return@optionIf player.questRepository.getQuest(Quests.THE_GOLEM).hasRequirements(player)
        }.playerl("Shall I try to repair you?").npcl(FaceAnim.OLD_BOWS_HEAD_SAD, "Repairs... needed...")
            .endWith { _, player ->
                if (player.questRepository.getStage(Quests.THE_GOLEM) < 1) {
                    setQuestStage(player, Quests.THE_GOLEM, 1)
                }
            }
        opt1.option("I'm not going to find a conversation here!").playerl("I'm not going to find a conversation here!")
            .end()
        b.onQuestStages(Quests.THE_GOLEM, 1).npcl(FaceAnim.OLD_BOWS_HEAD_SAD, "Repairs... needed...").end()
        b.onQuestStages(Quests.THE_GOLEM, 2).npcl(FaceAnim.OLD_BOWS_HEAD_SAD, "Damage repaired...")
        ...
        b.onQuestStages(Quests.THE_GOLEM, 3).npcl(
            FaceAnim.OLD_BOWS_HEAD_SAD,
            "The demon is still wounded. You must open the portal so that I can strike the final blow and complete my task.",
        ).end()
        b.onQuestStages(Quests.THE_GOLEM, 4).npcl(
            FaceAnim.OLD_BOWS_HEAD_SAD,
            "My task is incomplete. You must open the portal so I can defeat the great demon.",
        ).playerl("It's ok, the demon is dead!").npcl(FaceAnim.OLD_BOWS_HEAD_SAD, "The demon must be defeated...")
        ...
        b.onQuestStages(Quests.THE_GOLEM, 5).npcl(FaceAnim.OLD_BOWS_HEAD_SAD, "Task incomplete.")
            .playerl("Oh, how am I going to convince you?")
            .endWith { _, player -> setQuestStage(player, Quests.THE_GOLEM, 6) }
        b.onQuestStages(Quests.THE_GOLEM, 6, 7).npcl(
            FaceAnim.OLD_BOWS_HEAD_SAD,
            "My task is incomplete. You must open the portal so I can defeat the great demon.",
        ).playerl("I already told you, he's dead!").npcl(FaceAnim.OLD_BOWS_HEAD_SAD, "Task incomplete.")
            .playerl("Oh, how am I going to convince you?").endWith { _, player ->
...
class CuratorHaigHalenGolemDialogue : DialogueBuilderFile() {
    override fun create(b: DialogueBuilder) {
        val opt1 = b.onQuestStages(Quests.THE_GOLEM, 3).npcl(
            FaceAnim.OLD_BOWS_HEAD_SAD,
            ...
            .playerl("No, I need to take it away with me.").npcl(FaceAnim.OLD_BOWS_HEAD_SAD, "What do you want it for?")
            .options()
        opt1.option("I want to open a portal to the lair of an elder-demon.")
            .playerl("I want to open a portal to the lair of an elder-demon.")
            .npcl(FaceAnim.OLD_BOWS_HEAD_SAD, "Good heavens! I'd never let you do such a dangerous thing.").end()
        opt1.option("Well, I, er, just want it.").playerl("Well, I, er, just want it.").end()
    }
}
