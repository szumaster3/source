...
class DhalakDialogue(
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        // Calculate the correct NPC based on the Zaros path progress.
        val correctNPC = NPC(2384 + getAttribute(player, GameAttributes.ZAROS_PATH_SEQUENCE, 0))

        // Talking without Ghostspeak Amulet in equipment.
        if (!inEquipment(player, Items.GHOSTSPEAK_AMULET_552)) {
            end()
            openDialogue(player, RandomDialogue(), correctNPC)
            return true
        }

        /*
         * If the player is talking to the wrong NPC.
         */

        if (npc.id != correctNPC.id) {
            end()
            openDialogue(player, WrongLocationDialogue(), correctNPC)
            return true
        }
        ...
        if (CurseOfZaros.hasComplete(player)) {
            ...
            if (!CurseOfZaros.hasItems(player, item)) {
                player(
                    FaceAnim.SAD, "Could I have that hat again? I seem to have", "misplaced it somewhere..."
                ).also { stage = 39 }
            } else {
        ...
        if (getAttribute(player, GameAttributes.ZAROS_NPC_TALK + ":" + npc.id, false)) {
            player("Where would you suggest I look for Viggora?").also {
    ...
    override fun handle(
        ...
        when (stage) {
            ...
            2 -> if (getStatLevel(player, Skills.MAGIC) < 99) {
            ...
            10 -> when (buttonId) {
                ...
                2 -> player("Well, that's all fascinating, but I just don't particularly care.", "Bye-bye.").also {
                    stage = END_DIALOGUE
                }
            }
            ...
            36 -> player("Where would you suggest I look for Viggora?").also {
                stage = (100 + getAttribute(player, GameAttributes.ZAROS_PATH_SEQUENCE, 0))
            }
            ...
            101 -> npc(
                ...
                "Ah, the evil swordsman Viggora...",
                "Paddewwa was where he fought many battles, perhaps",
                "he has returned to one of his old haunts?"
            ).also { stage = 37 }
            // Second path dialogue.
            102 -> npc(
                FaceAnim.HALF_GUILTY,
                "Ah, the evil swordsman Viggora...",
                "A rogue like him would probably flock to his own kind."
            ).also { stage = 37 }
            // Third path dialogue.
            103 -> npc(
                FaceAnim.HALF_GUILTY,
                "Ah, the evil swordsman Viggora...",
                "Perhaps he has returned to his castle in the dark lands?"
            ).also { stage = 37 }
        }
        return true
    }
