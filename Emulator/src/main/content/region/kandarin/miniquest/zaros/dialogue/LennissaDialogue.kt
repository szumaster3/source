...
class LennissaDialogue(
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (CurseOfZaros.hasComplete(player)) {
            ...
            if (!CurseOfZaros.hasItems(player, item)) {
                player(
                    FaceAnim.SAD, "Could I have that robe bottom again? I seem to have", "misplaced it somewhere..."
                ).also { stage = 57 }
            } else {
        ...
        if (getAttribute(player, GameAttributes.ZAROS_NPC_TALK + ":" + npc.id, false)) {
            player("Where would I be able to find this Dhalak then?").also {
    ...
    override fun handle(
        ...
        when (stage) {
            1 -> npc(
                "Who are you?", "Where did you hear my name?", "How comes it that you can see and speak to me?"
            ).also { stage++ }
            ...
            57 -> {
                ...
                npc(
                    FaceAnim.CALM_TALK, "Certainly, I am not sure how, but it returned to me by", "some magic or other."
                )
            ...
            202 -> npc(FaceAnim.HALF_GUILTY, "I would look for a library to find him if I were you.").also {
                stage = 55
            }
            // Third path.
            103 -> npc(
