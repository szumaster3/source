...
class RennardDialogue(
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (CurseOfZaros.hasComplete(player)) {
            ...
            if (!CurseOfZaros.hasItems(player, item)) {
                player(
                    FaceAnim.SAD, "I lost those gloves you gave me...", "Can I have some more please?"
                ).also { stage = 38 }
            } else {
        ...
        if (getAttribute(player, GameAttributes.ZAROS_NPC_TALK + ":" + npc.id, false)) {
            player("Where can I find Kharrim then?").also {
    ...
    override fun handle(
        ...
        when (stage) {
            ...
            34 -> player("Where can I find Kharrim then?").also {
                stage = (100 + getAttribute(player, GameAttributes.ZAROS_PATH_SEQUENCE, 0))
            }
            ...
            202 -> npc(FaceAnim.HALF_GUILTY, "You might want to check around there.").also { stage = 35 }
            // Third path.
            103 -> npc(
                FaceAnim.HALF_GUILTY,
                "Kharrim the messenger...",
