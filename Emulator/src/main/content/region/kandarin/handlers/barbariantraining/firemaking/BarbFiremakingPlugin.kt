package content.region.kandarin.handlers.barbariantraining.firemaking
...
@Initializable
class BarbFiremakingPlugin : UseWithHandler(*tools) {

    override fun newInstance(arg: Any?): Plugin<Any> {
        tools.forEach { tool ->
            addHandler(tool, ITEM_TYPE, this)
        }
        logs.forEach { log ->
            addHandler(log, ITEM_TYPE, this)
        }
        return this
    }
    ...
    override fun handle(event: NodeUsageEvent): Boolean {
        ...
        val tool = if (tools.contains(first.id)) first else if (tools.contains(second.id)) second else return false
        val log = if (logs.contains(first.id)) first else if (logs.contains(second.id)) second else return false
        ...
        if (tool.name.contains("CRYSTAL BOW", true) || tool.name.contains("CRYSTAL SHIELD", true)) {
            sendDialogue(
                player,
        ...
        if (tool.id == Items.COMP_OGRE_BOW_4827 || tool.id == Items.OGRE_BOW_2883) {
            sendDialogue(
                player,
        ...
        submitIndividualPulse(player, BarbFiremakingPulse(player, log.asItem(), null))
        return true
    }
