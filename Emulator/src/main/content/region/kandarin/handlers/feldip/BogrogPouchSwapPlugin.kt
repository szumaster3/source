...
@Initializable
class BogrogPouchSwapPlugin : OptionHandler() {

    companion object {
        private const val NPC_ID = NPCs.BOGROG_4472
    ...
    override fun newInstance(arg: Any?): Plugin<Any?> {
        ...
        NPCDefinition.forId(NPC_ID).handlers["option:swap"] = this
        return this
    }

    override fun handle(player: Player, node: Node, option: String): Boolean {
        if (node.id != NPC_ID) return false

        return when (option.lowercase()) {
            "talk-to" -> {
                openDialogue(player, node.asNpc())
                true
            }
            "swap" -> {
                openSwap(player)
