...
class ProfessorDialogueFile : DialogueFile() {
    override fun handle(
        ...
        when (stage) {
            ...
            64 ->
                sendNPCDialogue(player!!, NPCs.OBSERVATORY_ASSISTANT_6118, "It'll need to be made to an exact shape and size.").also { stage++ }

            65 -> npcl("Well, obviously, hence why we have a lens mould.").also { stage++ }
            66 ->
                sendNPCDialogue(player!!, NPCs.OBSERVATORY_ASSISTANT_6118, "Not any more. One of those goblins took it.").also { stage++ }

            67 ->
                npcl(FaceAnim.SAD, "Great, just what I need. ${player!!.username}, I don't suppose you could find it?").also { stage++ }
            ...
            70 ->
                sendNPCDialogue(player!!, NPCs.OBSERVATORY_ASSISTANT_6118, "What have I done to deserve this?").also { stage++ }
            ...
            74 ->
                npc(FaceAnim.HALF_THINKING, "Well, from the smell I'd guess cooking some vile", "concoction.").also { stage++ }

            75 -> player("Wow, good guess. Well, here you go.").also { stage++ }
            76 ->
                sendNPCDialogue(player!!, NPCs.OBSERVATORY_ASSISTANT_6118, "Please don't give that to him. Last time he tried any Crafting, I had to spend a week cleaning up after the explosion.").also { stage++ }
            ...
            87 ->
                npc("Would you accompany me to the Observatory? You", "simply must see the telescope in operation.").also { stage++ }

            88 -> player("Sounds interesting. Count me in.").also { stage++ }
            89 ->
                npc("Superb. You'll have to go via the dungeon under the", "goblin settlement, seeing as the bridge is broken. You'll", "find stairs up to the Observatory from there.").also { stage++ }
            ...
            93 ->
                npc("Certainly is. Please, take a look through the telescope", "and tell me what you see.").also { stage = END_DIALOGUE }

            94 -> player("I've had a look through the telescope.").also { stage++ }
            95 ->
                npc("What did you see? If you're not sure, you can find", "out by looking at the star charts dotted around the", "walls downstairs.").also { stage++ }
        ...
        when (stage) {
            ...
            1 ->
                player("Aquarius!").also {
                    setAttribute(
                        player!!,
                        ObservatoryPlugin.CONSTELLATION_ATTRIBUTE,
                        1,
                    )
                    stage = 16
                }
