...
class ProfessorDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> sendDialogue(player, "The professor carries on with his studies.").also { stage = 22 }
            4 -> when (buttonId) {
            ...
            7 -> npcl(FaceAnim.NEUTRAL, "Okay, no problem. See you again.").also { stage = 22 }
            8 -> npcl(FaceAnim.NEUTRAL, "Not right now, but the stars may hold a secret for you.").also { stage = 22 }

            9 -> options("I've lost my chart.", "How do these work again?").also { stage++ }
            10 -> when (buttonId) {
                1 -> player("I've lost my chart.").also { stage = 19 }
