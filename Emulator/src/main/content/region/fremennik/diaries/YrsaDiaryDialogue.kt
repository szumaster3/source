package content.region.fremennik.diaries
...
class YrsaDiaryDialogue : DialogueFile() {
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            0 -> {
                if (player!!.achievementDiaryManager.getDiary(DiaryType.FREMENNIK)!!
                        .isComplete(1, true) && !Diary.hasClaimedLevelRewards(player!!, DiaryType.FREMENNIK, 1)
                ) {
            ...
            10 -> player(
                "I've completed all of the medium tasks in my Fremennik", "Achievement Diary."
            ).also { stage++ }
            ...
            11 -> {
                if (freeSlots(player!!) < 1) {
                    ...
                    npc(
                        FaceAnim.FRIENDLY, "Although, you'll have to come back when you've the space", "to take them."
                    ).also {
                        stage = END_DIALOGUE
                    }
                } else {
