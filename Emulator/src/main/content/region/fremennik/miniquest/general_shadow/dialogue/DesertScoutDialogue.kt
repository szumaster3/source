package content.region.fremennik.miniquest.general_shadow.dialogue

import content.region.fremennik.miniquest.general_shadow.plugin.GeneralShadow
...
class DesertScoutDialogue(player: Player? = null, ) : Dialogue(player) {
    override fun open(vararg args: Any?): Boolean {
        return when {
            ...
            GeneralShadow.getShadowProgress(player) == 0 && getAttribute(player, GeneralShadow.GS_TRUSTWORTHY, false) -> {
    ...
    override fun handle(interfaceId: Int, buttonId: Int, ): Boolean {
        when (stage) {
            ...
            14 -> if (isQuestComplete(player, Quests.DESERT_TREASURE)) {
