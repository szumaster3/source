    ...
    override fun defineListeners() {
        ...
        on(ENTER_ENTRANCE, IntType.SCENERY, "open") { player, node ->
            ...
            queueScript(player, 3, QueueStrength.SOFT) {
                player.teleporter.send(Location.create(2394, 10300, 1), TeleportManager.TeleportType.INSTANT)
        ...
        on(EXIT_ENTRANCE, IntType.SCENERY, "exit") { player, _ ->
            player.teleporter.send(Location.create(2317, 3893, 0), TeleportManager.TeleportType.INSTANT)
            return@on true
        }
        ...
        on(Scenery.STONE_LADDER_21592, IntType.SCENERY, "climb-up") { player, _ ->
            player.teleporter.send(Location.create(2401, 3888, 0), TeleportManager.TeleportType.INSTANT)
            return@on true
        }
