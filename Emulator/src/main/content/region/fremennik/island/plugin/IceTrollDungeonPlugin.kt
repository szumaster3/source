...
class IceTrollDungeonPlugin : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(ROPE_BRIDGE, IntType.SCENERY, "walk-across") { player, node ->
            ...
            forceMove(player, player.location, player.location.transform(destination.toDirection(), 5), 30, 180, direction)
        ...
        on(ENTER_ENTRANCE, IntType.SCENERY, "open") { player, node ->
            ...
            queueScript(player, 3, QueueStrength.SOFT) {
                player.teleporter.send(Location.create(2394, 10300, 1))
        ...
        on(EXIT_ENTRANCE, IntType.SCENERY, "exit") { player, _ ->
            player.teleporter.send(Location.create(2317, 3893, 0))
            return@on true
        }
        ...
        on(Scenery.STONE_LADDER_21592, IntType.SCENERY, "climb-up") { player, _ ->
            player.teleporter.send(Location.create(2401, 3888, 0))
            return@on true
        }
