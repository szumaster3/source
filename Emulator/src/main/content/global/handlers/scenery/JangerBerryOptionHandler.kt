...
class JangerBerryOptionHandler : OptionHandler() {
    override fun newInstance(arg: Any?): Plugin<Any> {
        ...
        /*
         * Handles interaction with tree rope.
         */

        UseWithHandler.addHandler(Scenery.BRANCH_2326, UseWithHandler.OBJECT_TYPE, object : UseWithHandler(Items.ROPE_954) {
    ...
    override fun handle(player: Player, node: Node, option: String, ): Boolean {
        ...
        forceWalk(player, 0, player.location, end, Animation.create(Animations.ROPE_SWING_751), 20, 22.0, "You skillfully swing across.", 1)
        return true
    }

    override fun getDestination(node: Node, n: Node, ): Location = if (n.id == Scenery.ROPESWING_2324) Location(2511, 3092, 0) else Location(2501, 3087, 0)
}
