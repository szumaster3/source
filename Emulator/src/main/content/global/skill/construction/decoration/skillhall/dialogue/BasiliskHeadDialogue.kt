...
class BasiliskHeadDialogue : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            ...
            4 -> playerl(FaceAnim.FRIENDLY, "You're a $randomWordA $randomWordB $randomWordC $randomWordD.").also { stage++ }
            ...
            22 -> npcl(FaceAnim.CHILD_FRIENDLY, "Of course you won't do it again, you can only kill me once.").also { stage++ }
            23 -> options("That's why I won't do it again.", "But I won't do it to any other basilisks.").also { stage++ }
            24 -> when (buttonID) {
                1 -> playerl(FaceAnim.FRIENDLY, "That's why I won't do it again! There's be no point!").also { stage = 15 }
            ...
            29 -> npcl(FaceAnim.CHILD_FRIENDLY, "All right then. Apology accepted. Now leave me alone.").also { stage = END_DIALOGUE }
            30 -> npcl(FaceAnim.CHILD_FRIENDLY, "Apart from being dead and stuffed and hanging on a wall, you mean?").also { stage++ }
            31 -> playerl(FaceAnim.FRIENDLY, "Uh... yeah, apart from that are you okay?").also { stage++ }
            32 -> npcl(FaceAnim.CHILD_FRIENDLY, "Actually there's something blocking my view of the far wall.").also { stage++ }
