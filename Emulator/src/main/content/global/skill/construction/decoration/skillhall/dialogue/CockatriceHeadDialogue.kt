...
class CockatriceHeadDialogue : DialogueFile() {
    override fun handle(
        ...
        when (stage) {
            ...
            5 -> npcl(FaceAnim.CHILD_FRIENDLY, "Why do the Slayer Masters all pick on poor cockatrice?").also { stage++ }
            6 -> playerl(FaceAnim.FRIENDLY, "They pick on lots of other creatures too.").also { stage++ }
            7 -> npcl(FaceAnim.CHILD_FRIENDLY, "Then mount one of them on your wall and let poor cockatrice rest in peace!").also { stage = END_DIALOGUE }
            8 -> npcl(FaceAnim.CHILD_FRIENDLY, "Another cockatrice falls victim to the dreaded mirror shield!").also { stage++ }
            9 -> playerl(FaceAnim.FRIENDLY, "Don't take it personally. You look good on my wall.").also { stage++ }
            10 -> npcl(FaceAnim.CHILD_FRIENDLY, "I don't care! I think I looked better with a body!").also { stage = END_DIALOGUE }
            11 -> npcl(FaceAnim.CHILD_FRIENDLY, "You dirty rotten swine, you!").also { stage++ }
            12 -> playerl(FaceAnim.FRIENDLY, "Steady on.").also { stage++ }
            13 -> npcl(FaceAnim.CHILD_FRIENDLY, "I will kill you with my paralyzing-type magic eyes look!").also { stage++ }
            14 -> npcl(FaceAnim.CHILD_FRIENDLY, "Dots appear in air between eyes and victim. Dot! Dot! Dotty!").also { stage++ }
            15 -> playerl(FaceAnim.FRIENDLY, "Er, nothing's happening...").also { stage++ }
            16 -> npcl(FaceAnim.CHILD_FRIENDLY, "Concentrates mental power. Eyes narrow beak clenches veins on head stand out.").also { stage++ }
            17 -> npcl(FaceAnim.CHILD_FRIENDLY, "Strain!").also { stage++ }
            18 -> playerl(FaceAnim.FRIENDLY, "You're dead, cockatrice. Your eyes are glass beads. It won't work.").also { stage++ }
            ...
            100 -> npc(FaceAnim.CHILD_FRIENDLY, "House owner deaded me! That wasn't very nice!").also { stage = END_DIALOGUE }
        }
    }
}
