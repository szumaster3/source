...
open class CookingPulse(
    ...
    open fun isBurned(
        ...
        if (hasGauntlets && CookableItem.gauntletValues.containsKey(food)) {
            val successValues = CookableItem.gauntletValues[food]
            low = successValues!![0]
            high = successValues[1]
        } else if (scenery.id == LUMBRIDGE_RANGE) {
            val successValues =
                CookableItem.lumbridgeRangeValues.getOrDefault(food, intArrayOf(item!!.lowRange, item.highRange))
            low = successValues[0]
            high = successValues[1]
        } else {
            ...
            if (!burned) {
                ...
                player.dispatch(
                    ResourceProducedEvent(
                        itemId = CookableItem.getBurnt(initial).id,
                        amount = 1,
                        source = sceneryId!!,
