...
class EmptyOptionPlugin : InteractionListener {
    ...
    /**
     * Represents items that can be emptied.
     */
    enum class EmptyItem(var fullId: Int, var emptyId: Int, var emptyMessage: String, var audioId: Int = -1, ) {
        ...
        FISHBOWL_WITH_SEAWEED(Items.FISHBOWL_6669, Items.FISHBOWL_6667, "You empty the contents of the fishbowl onto the floor."),
        ...
        companion object {
            val emptyItemMap = mutableMapOf<Int, Int>()
            val emptyMessageMap = mutableMapOf<Int, String>()
            val emptyAudioMap = mutableMapOf<Int, Int>()
            val emptyItemList = mutableListOf<Int>()

            init {
                values().forEach {
                    emptyItemMap[it.fullId] = it.emptyId
                    emptyMessageMap[it.fullId] = it.emptyMessage
                    emptyAudioMap[it.fullId] = it.audioId
                    emptyItemList.add(it.fullId)
                }

                ItemDefinition.definitions.values.forEach { itemDef ->
                    val name = itemDef.name.lowercase()
                    if (listOf("potion", "brew", "poison", "serum", "cure", "mix", "balm").any { name.contains(it) }) {
                        emptyItemList.add(itemDef.id)
                    }
                }
            }
