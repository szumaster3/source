...
/**
 * Represents the Bartender (East Varrock) dialogue.
 */
@Initializable
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> when (buttonId) {
                ...
                2 -> player(FaceAnim.ASKING, "Can you recommend where an adventurer might make", "his fortune?").also { stage = 20 }
            ...
            20 -> npc(FaceAnim.NEUTRAL, "Ooh I don't know if I should be giving away information,", "makes the computer game too easy.").also { stage++ }
            ...
            160 -> npc(FaceAnim.THINKING, "This world around us... is a computer game.... called", settings!!.name + ".").also { stage++ }
            161 -> player(FaceAnim.HALF_THINKING, "Nope, still don't understand what you are talking about.", "What's a computer?").also { stage++ }
            162 -> npc(FaceAnim.THINKING, "It's a sort of magic box thing, which can do all sorts of", "stuff.").also { stage++ }
            163 -> player(FaceAnim.WORRIED, "I give up. You're obviously completely mad.").also { stage = END_DIALOGUE }
            30 -> npc(FaceAnim.FRIENDLY, "Well, there's the sword shop across the road, or there's", "also all sorts of shops up around the market.").also { stage = END_DIALOGUE }
            170 -> npc(FaceAnim.NEUTRAL, "Go and talk to the bartender at the Holly Boar Inn, he", "doesn't seem to mind giving away clues.").also { stage = END_DIALOGUE }
        }
        return true
    }
