package content.region.kandarin.miniquest.ctr.dialogue
...
class SquireDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        when {
            getAttribute(player, GameAttributes.KW_BEGIN, false) -> npc(
                "Good day, my lord. Is there anything I can do", "for you?"
            ).also {
                stage = 14
            }

            getAttribute(player, GameAttributes.KW_COMPLETE, false) -> npc(
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> npc(
                FaceAnim.HAPPY, "That door leads to the training ground for the Knights", "of the Round Table."
            ).also { stage++ }

            2 -> playerl(FaceAnim.FRIENDLY, "Can I go in there?").also { stage++ }
            3 -> npc(
                "Of course, my lord. But I must tell you the rules first.", "Would you like to hear them now?"
            ).also { stage++ }
            ...
            28 -> npc("If you do not want your respawn point set to Camelot,", "Merlin can change it for you.").also {
                stage = END_DIALOGUE
            }

            29 -> player(FaceAnim.HALF_ASKING, "Thank you. Can I go back inside?").also { stage++ }
            30 -> npcl(FaceAnim.NEUTRAL, "I'm afraid not. Once you have won, you cannot re-enter.").also {
                stage = END_DIALOGUE
            }
        }
        return true
