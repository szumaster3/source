...
class PrivatePaldoDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> npcl(FaceAnim.HALF_GUILTY, "A barbarian! Word is he's been smashing through waves of Penance faster than anyone we've ever seen.").also { stage++ }
            3 -> npcl(FaceAnim.HALF_GUILTY, "That's why we've decided to battle harder set of waves to challenge the veterans.").also { stage++ }
            4 -> playerl(FaceAnim.HALF_GUILTY, "Well, I suppose that's a good idea.").also { stage++ }
            5 -> npcl(FaceAnim.HALF_GUILTY, "You should try them yourself. Put your skills to the test against the Penance King!").also { stage = END_DIALOGUE }
        }
        return true
    }
