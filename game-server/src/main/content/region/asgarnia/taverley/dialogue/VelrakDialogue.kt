...
class VelrakDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            10 -> npcl(FaceAnim.HALF_THINKING, "Well, this dungeon was quite good to explore...until I got captured, anyway. I was given a key to an inner part of this dungeon by a mysterious stranger!").also { stage++ }
            11 -> npcl(FaceAnim.NEUTRAL, "It's rather tough for me to get that far into the dungeon however. I just keep getting captured! Would you like to give it a go?").also { stage++ }
            ...
            14 -> sendItemDialogue(player, Items.DUSTY_KEY_1590, "Velrak reaches somewhere mysterious and passes you a key.").also {
                addItem(player, Items.DUSTY_KEY_1590, 1)
                stage = END_DIALOGUE
            }
            15 -> npcl(FaceAnim.FRIENDLY, "I don't blame you!").also { stage = END_DIALOGUE }
            20 -> npcl(FaceAnim.HALF_GUILTY, "Well, not really. The Black Knights took all of my stuff before throwing me in here to rot!").also { stage = 0 }
            100 -> npcl(FaceAnim.HALF_THINKING, "Yes... I'm still plucking up the courage to run out past those Black Knights.").also { stage++ }
