...
class BardurDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            5 -> npcl(FaceAnim.NEUTRAL, "Listen, ${FremennikTrials.getFremennikName(player)}. I'm sorry about the way I acted while you were exiled.").also { stage++ }
            ...
            8 -> npcl(FaceAnim.NEUTRAL, "I have been here all the long day, slaughtering the daggermouth spawns so that none may pass!").also { stage++ }
            9 -> npcl(FaceAnim.NEUTRAL, "My sword arm grows weary, and the blood of my foes hangs heavy upon my clothing!").also { stage++ }
            10 -> playerl(FaceAnim.FRIENDLY, "Can you tell me anything about the caves down here?").also { stage++ }
            11 -> npcl(FaceAnim.NEUTRAL, "Aye, that I can ${FremennikTrials.getFremennikName(player)}! Yonder lies the lair of the daggermouth kings, the three beasts of legend!").also { stage++ }
            12 -> npcl(FaceAnim.NEUTRAL, "I train my skills upon its foul brood, to prepare myself for the fight ahead!").also { stage++ }
            13 -> playerl(FaceAnim.FRIENDLY, "Okay, thanks Bardur. Do you have any food with you?").also { stage++ }
            14 -> npcl(FaceAnim.NEUTRAL, "Ah, you did not come prepared ${FremennikTrials.getFremennikName(player)}?").also { stage++ }
            15 -> npcl(FaceAnim.NEUTRAL, "I have nothing to spare, but my equipment grows weaker under the onslaught of the dagger-mouth spawns!").also { stage++ }
