...
public class WoodcuttingPulse extends Pulse {
    ...
    public void animate() {
        if(!player.getAnimator().isAnimating()) {
            player.animate(Animation.create(SkillingTool.getAxe(player).getAnimation()));
    ...
    public boolean reward() {
        ...
        if (node.getId() == 10041) {
            player.getDialogueInterpreter().sendDialogues(2574, FaceAnim.FURIOUS, RandomFunction.random(2) == 1 ? "You'll blow my cover! I'm meant to be hidden!" : "Will you stop that?");
            return true;
        }
        if (!checkReward(SkillingTool.getAxe(player))) {
            return false;
        }

        // 20% chance to auto burn logs when using "inferno adze" item
        if (SkillingTool.getAxe(player).getId() == 13661 && RandomFunction.random(100) < 25) {
